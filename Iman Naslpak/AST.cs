...
namespace AST
{
    ...
    /* Mir Iman Naslpak */
    public class SwitchStatement : Statements
    {
        private Expression SwitchExpression;
        private List<SwitchBlocks> switchblock;
        public SwitchStatement (Expression SwitchExpression, List<SwitchBlocks> switchblock) { this.SwitchExpression = SwitchExpression; this.switchblock = switchblock; }
    }
    public class SwitchBlocks : Node
    {
        private SwitchLabel SwitchLabel;
        private Statements SwitchStatement;
        public SwitchBlocks (SwitchLabel SwitchLabel, Statements SwitchStatement) { this.SwitchLabel = SwitchLabel; this.SwitchStatement = SwitchStatement; }
    }
    public class SwitchLabel : Node
    {
        ...
        public SwitchLabel (SwitchType switchtype, Expression CaseExpression) { this.switchtype = switchtype; this.CaseExpression = CaseExpression; EnumConstantname = null; }
        public SwitchLabel (SwitchType switchtype, string EnumConstantname) { this.switchtype = switchtype; this.EnumConstantname = EnumConstantname; CaseExpression = null; }
        public SwitchLabel (SwitchType switchtype) { this.switchtype = switchtype; EnumConstantname = null; CaseExpression = null; }
    }
    ...
    public class program
    {
        static void Main()
        {
            var program = new CompilationUnit(new List<TypeDeclaration> { new ClassDeclaration(new List<ClassModifier> {ClassModifier.Public},
                ...
                new List<FormalParameter> {new FormalParameter(new ArrayType(new NamedType("String Array")),"args") }, 
                new List<Statements> {new LocalVariableDeclaration(new NumericType(new NamedType("int")),"x"),
                    new AssignmentExpression( new ExpressionName("x"),'+', new PrimaryExpression(42)),
                    new SwitchStatement(new PrimaryExpression(42),new List<SwitchBlocks>
                    { new SwitchBlocks(new SwitchLabel(SwitchType.Case,new ExpressionName("A")),new LocalVariableDeclaration(new NumericType(new NamedType("Byte")),"A")),
                        new SwitchBlocks(new SwitchLabel(SwitchType.Case,"B"),new LocalVariableDeclaration(new NumericType(new NamedType("int")),"B")),
                        new SwitchBlocks(new SwitchLabel(SwitchType.Default),new AssignmentExpression( new ExpressionName("x"),'+', new PrimaryExpression(42))) })})})});
            program.DumpValue(1);
        }
    }
}
